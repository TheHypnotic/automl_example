...
class MedicalModelTrainer:
    ...
    def load_dataset(self, file_path):
        ...
        try:
            # Try to load directly as a dictionary with train/test keys
            df = pd.read_json(file_path)
            self.dataset = df
            
            # Check for valid structure - should have question and answer columns
            if "question" not in df.columns or "answer" not in df.columns:
                raise ValueError(f"Dataset at {file_path} must contain 'question' and 'answer' columns")
